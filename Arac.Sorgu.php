...
function aracBagyakaMi($a_Plaka)
{
    ...
    if ($sonbilgiler[0]["kullaniciid"]==$BAYAGA_ID) return 1;
	else return 0;
}
        ...
        if ($yon == "c") {
			...
			if(arackayittavarmi($arac_Plaka))
			{
				...
				if ($kul_id>0)  //müşteri ile ilişkili - aboneliği var olabilir.
				{
					...
					{
						if(aracBagyakaMi($a_Plaka)==1)
						{
							if($suredk>$BAYAGA_SURE)
							{
								sorguDb("update db_araclar set tipID='".$BAYAGA_UCRET_ID."' , odeme_durum=0 where plaka='$arac_Plaka'");
